...
function firstElement2<Type22>(arr: Type22[]): Type22 | undefined {
    return arr[0];
}
...
function map23<Input, Output>(arr: Input[], func: (arg: Input) => Output): Output[] {
    return arr.map(func);
}
   
const parsed = map23(["1", "2", "3"], (n) => parseInt(n));
console.log(parsed);